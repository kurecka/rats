...
void register_pareto_uct(py::module &m, const T& agent_type, std::string name) {
    py::class_<ts::pareto_uct<S, A>>(m, name.c_str(), agent_type)
        .def(py::init<environment_handler<S, A>, int, float, float, float, float, int, int, float, float, bool, int>(),
        "handler"_a, "max_depth"_a, "risk_thd"_a, "gamma"_a, "gammap"_a = 1, "max_disc_penalty"_a = 1,
