...
py::class_<environment<S, A>, std::shared_ptr<environment<S, A>>> register_environment(py::module& m, std::string type_name) {
    ...
    env_type
        ...
        .def("is_terminal", &environment<S, A>::is_terminal)
        .def("outcome_probabilities", &environment<S, A>::outcome_probabilities)
        .def("get_expected_reward", &environment<S, A>::get_expected_reward)
